...
/*
*   Metodo que recebe um boolean para
*   ativar o resize ou desativar
*/

Graph.prototype.resize = function(status){

    if(status == false){
        for (var i = 0; i <= this.nodes.length - 1; i++) {
            paper.freeTransform(dragndrop.nodes[i], { rotate:[ '', '', '' ], draw: [ 'bbox', '' ],scale:[ '', '', 'bboxCorners', '' ],drag:[ '', 'self' ], keepRatio: true });
        }
    } else{
        for (var i = 0; i <= this.nodes.length - 1; i++) {
            paper.freeTransform(dragndrop.nodes[i]).unplug();
