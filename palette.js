...
Palette.prototype.paletteUp = function(){
	if(!self.isInsideCanvas(this)){
        ...
        var t ='';
        //com a variavel scale, pode-se definir o numero de scales
        var scale=0;
        //conta linhas no "text"
        var countLines=0;
		...
		if(this.items[0].data('type') === 3 || this.items[0].data('type') === 4 || this.items[0].data('type') === 5){
		    ...
		    this.dblclick(function (){
                ...
                //é necessário para que não apareca o click me
                if(t=='Click me'){
                    t='';
                }
                
                t += prompt('Inserir dados:','');
                t += '\n';
                countLines+=1;
                //console.log(t);
                ...
                //primeiro scale
                if(t.length!=0 &&countLines==4){
                    //duplica tamanha da shape
                    self1.scale(2, 2, this.x, this.y);
                    //como a shape aumenta e o texto tambem, reduz-se o texto
                    this.scale(0.5,0.5,this.x,this.y);
                }//segundo scale
                else if(t.length!=0 &&countLines==8){
                    self1.scale(1.5, 1.5, this.x, this.y);
                    this.scale(0.8,0.8,this.x,this.y);
                }else if(t.length!=0 &&countLines==10){
                    self1.scale(1.5, 1.5, this.x, this.y);
                    this.scale(0.8,0.8,this.x,this.y);
                }else{
                    //nao faz nada
